...
| `shallow_cons_miss` | 500,000 | Concatenates one stable prefix with a freshly allocated numeric string each iteration. Every result is shallow and ≤64 bytes but has a distinct right-operand pointer, isolating the two-entry memo probe/publish miss cost. |
