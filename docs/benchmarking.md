...
| `relational_number` | 1,000,000 + 1,000,001 | Fused `JmpIfNotLt8` execution over Double/Int32 and Double/Double pairs. The comparison is each loop's sole condition, isolating §7.2.12 primitive-Number comparison from coercion and unrelated relational branches. |
| `relational_object` | 250,000 | Coercible-object/Double `JmpIfNotLt8` execution. A stable `valueOf` operand pins the shared §7.2.12 IsLessThan / §7.1.1 ToPrimitive path exactly once per comparison while exercising the primitive-Number guard's fallback routing; the outer loop uses truthiness rather than a second relational condition. |
