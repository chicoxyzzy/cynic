...
| `relational_object` | 250,000 | Coercible-object/Number `JmpIfNotLt8` execution. A stable `valueOf` operand pins the shared §7.2.12 IsLessThan / §7.1.1 ToPrimitive path exactly once per comparison; the outer loop uses truthiness rather than a second relational condition. |
