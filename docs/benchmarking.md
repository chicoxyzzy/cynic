...
| `relational_number` | 2×1,000,000 | Fused `JmpIfNotLt8` execution over Double/Int32 and Double/Double pairs. The comparison is each loop's sole condition, isolating §7.2.13 primitive-Number comparison from coercion and unrelated relational branches. |
| `relational_object` | 250,000 | Coercible-object/Number `JmpIfNotLt8` execution. A stable `valueOf` operand pins the shared §7.2.13 ToPrimitive path exactly once per comparison; the outer loop uses truthiness rather than a second relational condition. |
