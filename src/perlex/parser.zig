...
const Parser = struct {
    ...
    fn parseCharClass(self: *Parser) ParseError!*Node {
        ...
        // `[\S]`, `[\W]`, with or without a leading `^`) is exactly that
        // shorthand, or its complement. Under /u Perlex can't fold-complement
        // it inline (the parser isn't told `i`), but the equivalent
        // *standalone* class IS owned: at match time the VM extends the
        // positive set through the case-fold orbit and *then* negates, the
        // correct §22.2.2.7.3 reading (incl. the /iu word-char extension —
        // `[\W]/iu` excludes U+212A KELVIN, `[^\W]/iu` includes it). The
        // shorthand contributes one negation and a leading `^` another, so
        // the emitted class is negated iff there is no `^`. (Without /u it is
        // complemented in place below; a multi-member class with a `\D` `\S`
        // `\W` still takes the general path — see `parseClassMember`.)
        if ((self.unicode or self.unicode_sets) and
            self.peek() == '\\' and self.at(2) == ']')
        {
            ...
            if (base) |b| {
                ...
                return self.makeNode(.{ .class = .{ .negated = !negated, .ranges = b } });
            }
        }
