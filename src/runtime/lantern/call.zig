...
fn syntheticSetterDispatch(
    ...
    if (heap_mod.valueAsPlainObject(receiver)) |obj| {
        ...
        if (!obj.brand.extensible and !obj.ownDataContains(sa.key)) {
...
pub fn callValue(
    ...
    if (heap_mod.valueAsPlainObject(callee_v)) |po| {
        if ((po.brand.is_proxy) and po.brand.proxy_callable) {
            if (po.brand.proxy_revoked) {
        ...
        if (nt_proxy.brand.is_proxy) {
            ...
            switch (get_v) {
                ...
                .value => |v| {
                    ...
                    if (nt_proxy.brand.proxy_revoked or nt_proxy.getProxyHandler() == null) {
...
pub fn callJSFunction(
    ...
    // Lantern when caught.
    {
        // A fresh tier can hand a reconstructed frame to the canonical
        // safepoint before `runFrames` has registered this list. Keep the
        // registers rooted through that poll and any resulting unwind;
        // `runFrames` assumes ownership after this scope closes.
        var frame_scope = try JitFrameScope.init(realm, &frames);
