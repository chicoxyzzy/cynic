...
fn proxyConstructor(realm: *Realm, this_value: Value, args: []const Value) NativeError!Value {
    ...
    realm.heap.setProxyHandler(proxy, handler) catch return error.OutOfMemory;
    if (heap_mod.valueAsPlainObject(args[0])) |target| {
        realm.heap.setProxyTarget(proxy, target) catch return error.OutOfMemory;
...
fn callTrap(realm: *Realm, trap_fn: *@import("../function.zig").JSFunction, handler: *JSObject, args: []const Value) NativeError!Value {
    const outcome = lantern.callJSFunction(realm.allocator, realm, trap_fn, heap_mod.taggedObject(handler), args) catch |err| switch (err) {
