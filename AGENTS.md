...
- **Strict-only, non-browser-host target.** Cynic targets edge
  ...
  - **`eval` and runtime code construction** — off by
    ...
    **With `--allow=eval` the engine ships** (§19.2.1 PerformEval —
    ...
    so it *parses* every eval as strict code (no sloppy parser) — but
    the §19.2.1.3 variable-environment target still follows the body's
    `strictEval`: direct eval and a Use-Strict-Directive indirect body
    bind top-level `var` / function in the eval's own variable
    environment (direct eval reads the caller's scope but never injects
    bindings into it); a **non-strict indirect eval** (`(0,eval)("var
    x=1")`) binds them on the realm's global env, gated by §9.1.1.4.15/
    .16 CanDeclareGlobalVar / CanDeclareGlobalFunction (a TypeError when
